...
export default function LoginTEC({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      const res = await fetch('https://voxforge.com.br/api/login.php', {
        ...
        body: `cpf=${cleanCpf}&senha=${senha}&perfil=tecnico`,
      });
      ...
      if (json.status === 'ok') {
        ...
        navigation.reset({ index: 0, routes: [{ name: 'SplashSucessoTEC', params: { nome: json.nome } }] });
      } else {
      ...
      >
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <Animated.View style={[styles.header, { opacity: fade }]}>
            ...
            <Text style={styles.title}>{t('login_titulo2')}</Text>
          </Animated.View>
          ...
          <Animated.View style={[styles.form, { opacity: fade, transform: [{ translateY: slideUp }] }]}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('RecuperarSenhaTEC')}>
            ...
            <View style={styles.footer}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('CadastroTEC')}>
